...
function checkPlaylistLoaded(obj) {
	...
	if (jsPlaylist.length > 0) {
		...
		//Uncomment to add expose feature
		//player.addModelListener("STATE", "checkState");
	} else if ((jsPlaylist.length) === 1) {
		return null;
	} else {
...
function createPlaylist(obj){
	...
	playerDiv.after("<div class='jw_playlist_playlist_right'></div>");
...
/* This will also cause bugs in IE and Firefox when using this JavaScript playlist file
   Removed for functionality's sake
